...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const bgRGB = hexToRGB(backgroundColor || '#FFFFFF');
    const isLightBg = (bgRGB.r * 0.299 + bgRGB.g * 0.587 + bgRGB.b * 0.114) > 0.65 ? 1.0 : 0.0;
    const colorRGB = hexToRGB(color || '#FF6B00');

    renderer.setClearColor(new THREE.Color(backgroundColor || '#FFFFFF'), 1);
    ...
    mount.style.backgroundColor = backgroundColor || '#FFFFFF';
    ...
    const uniforms = {
      ...
      uColor: { value: new THREE.Vector3(colorRGB.r, colorRGB.g, colorRGB.b) },
      uBgColor: { value: new THREE.Vector3(bgRGB.r, bgRGB.g, bgRGB.b) },
      uIsLight: { value: isLightBg },
      uFade: { value: 1.0 }
    };
    ...
    const animate = () => {
      ...
      const dt = prevTime === 0 ? 0.016 : Math.max(0, t - prevTime);
      prevTime = t;
  ...
  useEffect(() => {
    ...
    const bgRGB = hexToRGB(backgroundColor || '#FFFFFF');
    const isLightBg = (bgRGB.r * 0.299 + bgRGB.g * 0.587 + bgRGB.b * 0.114) > 0.65 ? 1.0 : 0.0;
    if (uniforms.uBgColor) uniforms.uBgColor.value.set(bgRGB.r, bgRGB.g, bgRGB.b);
    if (uniforms.uIsLight) uniforms.uIsLight.value = isLightBg;
    if (uniforms.uFade) uniforms.uFade.value = 1.0;

    const canvas = rendererRef.current?.domElement;
    if (canvas) {
      const mount = mountRef.current;
      if (mount) mount.style.backgroundColor = backgroundColor || '#FFFFFF';
      canvas.style.filter = 'none';
      canvas.style.mixBlendMode = 'normal';
      if (rendererRef.current) {
        rendererRef.current.setClearColor(new THREE.Color(backgroundColor || '#FFFFFF'), 1);
      }
    }
  }, [
    wispDensity,
  ...
  return (
    <div
      ref={mountRef}
      className={`laser-flow-container w-full h-full relative pointer-events-none ${className || ''}`}
      style={style}
    />
  );
};
